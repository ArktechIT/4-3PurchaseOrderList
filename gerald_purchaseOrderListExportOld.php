<?php
	...
	$totalRecords = 0;
	$sql = "SELECT poNumber, supplierId, supplierType, poIssueDate, poTargetReceiveDate, poStatus, poCurrency, poInputDateTime FROM purchasing_podetailsnew ".$sqlFilter."";
	$tableContent = '';
		...
		if($queryLotNo->num_rows > 0)
		{
			while($resultLotNo = $queryLotNo->fetch_array())
			{
				...
				if($identifier==1)
				{
					...
					//~ $sql = "SELECT totalSurfaceClear FROM cadcam_subcondimension WHERE partId = ".$partId." LIMIT 1";
					//~ $querySubconDimension = $db->query($sql);
					//~ if($querySubconDimension->num_rows > 0)
					//~ {
						//~ $resultSubconDimension = $querySubconDimension->fetch_array();
						//~ $totalSurfaceClear = $resultSubconDimension['totalSurfaceClear'];
					//~ }
	...
	else
	{
		...
		if($querySupplyPo->num_rows > 0)
		{
			while($resultSupplyPo = $querySupplyPo->fetch_array())
			{
				...
				$supplyId = '';
				$supplyTypeRose = '';
				//$sql = "SELECT supplyId FROM purchasing_supplies WHERE listId = ".$listId." LIMIT 1";//by rose 2020-01-11
				$sql = "SELECT supplyId, supplyType FROM purchasing_supplies WHERE listId = ".$listId." LIMIT 1";
				...
				if($querySupplyId->num_rows > 0)
				{
					...
					$supplyTypeRose = $resultSupplyId['supplyType'];
					$poType = $resultSupplyId['supplyType'];
				}
				...
				//~ {
					...
					//~ {
						...
						//~ $itemName = $resultAccessories['itemName'];
						//~ $itemDescription = $resultAccessories['itemDescription'];
					//~ }
				//~ }
				//~ else if($poType == 3)
				//~ {
